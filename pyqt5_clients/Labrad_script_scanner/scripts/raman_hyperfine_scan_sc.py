...
DDS_wait_time1 = 0.01
DDS_wait_time2 = 0.01
#detection = 1 # 1ms . no too long . avoid far detuned population
...
class scan(experiment):
    ...
    required_parameters = [('cam_roi', 'height'),
                           ...
                           ('Cooling', 'pump1_pwr'),
                           ('Cooling', 'pump2_pwr'),
                           ('Raman', 'raman_sp_zeeman_pwr'),
                           ('Raman', 'raman_zeeman_resonance_freq'),
                           ('Raman', 'raman_zeeman_pi_time'),
                           ('RamanHyperfine', 'raman_hpf_detuning'),
                           ('RamanHyperfine', 'raman_hpf_pi_time'),
                           ('RamanHyperfine', 'raman_hpf_scanfreq'),
                           ('RamanHyperfine', 'raman_hpf_scantime'),
                           ('RamanHyperfine', 'scan_use_detuning'),
                           ('Raman', 'raman1_freq'),
                           ('Raman', 'raman2_freq'),
                           ('RamanHyperfine', 'raman_hpf_resonance_freq'),
                           ('Raman', 'raman1_pwr'),
                           ('Raman', 'raman2_pwr'),
                           ('Raman', 'DDS_wait_time'),
                           ('RamanHyperfine', 'raman_sp_hpf_pwr'),
                           ('RamanHyperfine', 'repetitions_every_point'),
                           ('RamanHyperfine', 'detection_time'),
                           ('scan_use_pmt', 'USEPMT')
                           ]
    USEPMT = False
    continue_flag = False
    ...
    def program_main_sequence(self, scanfreq, scantime):
        ...
        RamanSPAOM_zeeman_amp = self.parameters.Raman.raman_sp_zeeman_pwr
        raman_zeeman_pi_time = self.parameters.Raman.raman_zeeman_pi_time
        RamanSPAOM_zeeman_freq = self.parameters.Raman.raman_zeeman_resonance_freq

        Raman1AOM_freq = self.parameters.Raman.raman1_freq
        Raman2AOM_freq = self.parameters.Raman.raman2_freq
        Raman1AOM_amp = self.parameters.Raman.raman1_pwr
        Raman2AOM_amp = self.parameters.Raman.raman2_pwr
        RamanSPAOM_amp = self.parameters.RamanHyperfine.raman_sp_hpf_pwr

        DDS_wait_time = self.parameters.Raman.DDS_wait_time
        detection = self.parameters.RamanHyperfine.detection_time
        period1_start_at = 0.5
        #Raman_zeeman1_start_at = period1_start_at + pump1 + DDS_wait_time1
        #Raman_hpf_start_at = Raman_zeeman1_start_at + raman_zeeman_pi_time + DDS_wait_time2
        #Raman_zeeman2_start_at = Raman_hpf_start_at + scantime + DDS_wait_time2
        #detection_start_at = Raman_zeeman2_start_at + raman_zeeman_pi_time + DDS_wait_time2
        Raman_zeeman1_start_at = period1_start_at + pump1 + DDS_wait_time
        Raman_hpf_start_at = Raman_zeeman1_start_at + raman_zeeman_pi_time + DDS_wait_time
        Raman_zeeman2_start_at = Raman_hpf_start_at + scantime + DDS_wait_time
        detection_start_at = Raman_zeeman2_start_at + raman_zeeman_pi_time + DDS_wait_time
        # this is always needed if you want to start a new sequence
        self.pulser.new_sequence()
        ...
        DDS = [
            ...
            # raman_zeeman_pi_pulse
            # Raman1AOM
            ('Raman1 AOM', WithUnit(Raman_zeeman1_start_at, 'ms'), WithUnit(raman_zeeman_pi_time, 'ms'),
             WithUnit(Raman1AOM_freq, 'MHz'), WithUnit(Raman1AOM_amp, 'dBm'), WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),
             WithUnit(0, 'dB')),
            # Raman2AOM
            ('Raman2 AOM', WithUnit(Raman_zeeman1_start_at, 'ms'), WithUnit(raman_zeeman_pi_time, 'ms'),
             WithUnit(Raman2AOM_freq, 'MHz'), WithUnit(Raman2AOM_amp, 'dBm'), WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),
             WithUnit(0, 'dB')),
            # RamanSPAOM
            ('Raman SPAOM', WithUnit(Raman_zeeman1_start_at, 'ms'), WithUnit(raman_zeeman_pi_time, 'ms'),
             WithUnit(RamanSPAOM_zeeman_freq, 'MHz'), WithUnit(RamanSPAOM_zeeman_amp, 'dBm'), WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),
             WithUnit(0, 'dB')),

            # raman_hyperfine
            # Raman1AOM
            ('Raman1 AOM', WithUnit(Raman_hpf_start_at, 'ms'), WithUnit(scantime, 'ms'),
             WithUnit(Raman1AOM_freq, 'MHz'), WithUnit(Raman1AOM_amp, 'dBm'), WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),
             WithUnit(0, 'dB')),
            # Raman2AOM
            ('Raman2 AOM', WithUnit(Raman_hpf_start_at, 'ms'), WithUnit(scantime, 'ms'),
             WithUnit(Raman2AOM_freq, 'MHz'), WithUnit(Raman2AOM_amp, 'dBm'), WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),
             WithUnit(0, 'dB')),
            # RamanSPAOM
            ('Raman SPAOM', WithUnit(Raman_hpf_start_at, 'ms'), WithUnit(scantime, 'ms'),
             WithUnit(scanfreq, 'MHz'), WithUnit(RamanSPAOM_amp, 'dBm'), WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),
             WithUnit(0, 'dB')),

            # raman_zeeman_pi_pulse
            # Raman1AOM
            ('Raman1 AOM', WithUnit(Raman_zeeman2_start_at, 'ms'), WithUnit(raman_zeeman_pi_time, 'ms'),
             WithUnit(Raman1AOM_freq, 'MHz'), WithUnit(Raman1AOM_amp, 'dBm'), WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),
             WithUnit(0, 'dB')),
            # Raman2AOM
            ('Raman2 AOM', WithUnit(Raman_zeeman2_start_at, 'ms'), WithUnit(raman_zeeman_pi_time, 'ms'),
             WithUnit(Raman2AOM_freq, 'MHz'), WithUnit(Raman2AOM_amp, 'dBm'), WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),
             WithUnit(0, 'dB')),
            # RamanSPAOM
            ('Raman SPAOM', WithUnit(Raman_zeeman2_start_at, 'ms'), WithUnit(raman_zeeman_pi_time, 'ms'),
             WithUnit(RamanSPAOM_zeeman_freq, 'MHz'), WithUnit(RamanSPAOM_zeeman_amp, 'dBm'), WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),
             WithUnit(0, 'dB')),
    ...
    def initialize(self, cxn, context, ident):
        ...
        self.repetitions_every_point = self.parameters.RamanHyperfine.repetitions_every_point
    ...
    def run(self, cxn, context, scanvalue, units):
        ...
        usedetuning = self.parameters.RamanHyperfine.scan_use_detuning
        if usedetuning:
            detuning = self.parameters.RamanHyperfine.raman_hpf_detuning
            Scan_Freq = self.parameters.RamanHyperfine.raman_hpf_resonance_freq - detuning
        else:
            Scan_Freq = self.parameters.RamanHyperfine.raman_hpf_resonance_freq
        Scan_Time = self.parameters.RamanHyperfine.raman_hpf_pi_time
        ...
        if self.USEPMT == False:
            self.cam.start_sequence(
                ...
                self.repetitions_every_point-5)
        #self.cam.start_camera(r"C:\Users\Administrator\Desktop\Lab_control-Development\servers\control_instrument_servers\ZhongzhikeyiCam\test_tiff\test_hellowold.tiff")
        # you play the sequence for n times (n=100 here), i.e. number of repetitions per scan point.
        self.pulser.start_number(self.repetitions_every_point)
