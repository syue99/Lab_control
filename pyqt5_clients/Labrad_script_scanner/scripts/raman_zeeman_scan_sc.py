...
class scan(experiment):
    ...
    required_parameters = [('cam_roi', 'height'),
                           ...
                           ('Raman', 'raman2_pwr'),
                           ('Raman', 'DDS_wait_time'),
                           ('Raman', 'scan_use_detuning'),
                           ('Raman', 'raman_zeeman_detuning'),
                           ('Raman', 'raman_zeeman_scanfreq'),
                           ('Raman', 'raman_zeeman_scantime'),
                           ('Raman', 'repetitions_every_point'),
                           ('Raman', 'detection_time'),
                           ('scan_use_pmt', 'USEPMT')
                           ]
    USEPMT = False
    continue_flag = False
    ...
    def program_main_sequence(self, scanfreq, scantime):
        ...
        RamanSPAOM_amp = self.parameters.Raman.raman_sp_zeeman_pwr

        DDS_wait_time = self.parameters.Raman.DDS_wait_time
        detection = self.parameters.Raman.detection_time
        #detection = 0.1
        period1_start_at = 0.5
        #Raman_start_at = period1_start_at + pump1 + DDS_wait_time1
        #detection_start_at = Raman_start_at + scantime + DDS_wait_time2
        Raman_start_at = period1_start_at + pump1 + DDS_wait_time
        detection_start_at = Raman_start_at + scantime + DDS_wait_time
        # this is always needed if you want to start a new sequence
        self.pulser.new_sequence()
        ...
        DDS = [
            ...
            # raman_zeeman
            # Raman1AOM
            ('Raman1 AOM', WithUnit(Raman_start_at, 'ms'), WithUnit(scantime, 'ms'),
             WithUnit(Raman1AOM_freq, 'MHz'), WithUnit(Raman1AOM_amp, 'dBm'), WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),
             WithUnit(0, 'dB')),
            # Raman2AOM
            ('Raman2 AOM', WithUnit(Raman_start_at, 'ms'), WithUnit(scantime, 'ms'),
             WithUnit(Raman2AOM_freq, 'MHz'), WithUnit(Raman2AOM_amp, 'dBm'), WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),
             WithUnit(0, 'dB')),
            # RamanSPAOM
            ('Raman SPAOM', WithUnit(Raman_start_at, 'ms'), WithUnit(scantime, 'ms'),
    ...
    def initialize(self, cxn, context, ident):
        ...
        self.repetitions_every_point =int(self.parameters.Raman.repetitions_every_point)
    ...
    def run(self, cxn, context, scanvalue, units):
        ...
        usedetuning = self.parameters.Raman.scan_use_detuning
        if usedetuning:
            detuning = self.parameters.Raman.raman_zeeman_detuning
            Scan_Freq = self.parameters.Raman.raman_zeeman_resonance_freq - detuning
        else:
            Scan_Freq = self.parameters.Raman.raman_zeeman_resonance_freq
        Scan_Time = self.parameters.Raman.raman_zeeman_pi_time
