...
repetitions_every_point = 1  # No. of repeatitions when scanning every point
...
class scan(experiment):
    name = 'DopplerCoolingRepeat'
    parameter_name = "Doppler Cooling Time"
    ...
    def program_main_sequence(self):
        ...
        detection_start_at = doppler_cooling_at + doppler_time + DDS_wait_time1
        ...
        DDS = [
            ...
            # doppler
            ('Doppler AOM', WithUnit(doppler_cooling_at, 'ms'), WithUnit(doppler_time, 'ms'),
             WithUnit(detect_freq - 4.5, 'MHz'), WithUnit(doppler_amp, 'dBm'), WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),
             WithUnit(0, 'dB')),
    ...
    def run(self, cxn, context):
        ...
        if self.USEPMT == True:
            ...
            readout = avg_count / self.cam_var['width']
            #readout = avg_count
