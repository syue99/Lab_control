...
class scan(experiment):
    name = 'MicrowaveHyperfineRamseyScan'
    ...
    required_parameters = [('cam_roi', 'height'),
                           ...
                           ('Microwave', 'mw_pwr'),
                           ('MicrowaveHyperfine', 'hpf_resonance_freq'),
                           ('MicrowaveHyperfine', 'hpf_detuning'),
                           ('MicrowaveHyperfine', 'hpf_pi_time'),
                           ('MicrowaveHyperfine', 'hpf_pwr'),
                           ('MicrowaveHyperfine', 'Ramsey_evolution_time'),
                           ('scan_use_pmt', 'USEPMT')
                           ]
    USEPMT = False
    continue_flag = False
    ...
    def program_main_sequence(self, scanfreq, scantime):
        ...
        microwave_hyperfine_pi_time = self.parameters.MicrowaveHyperfine.hpf_pi_time
        microwave_freq = self.parameters.Microwave.mw_resonance_freq
        microwave_hyperfine_amp = self.parameters.MicrowaveHyperfine.hpf_pwr

        period1_start_at = 2
        microwave1_start_at = period1_start_at + pump1 + DDS_wait_time1
        microwave_hyperfine1_start = microwave1_start_at + microwave_pi_time + DDS_wait_time2
        microwave_hyperfine2_start = microwave_hyperfine1_start + microwave_hyperfine_pi_time / 2 + scantime
        microwave2_start_at = microwave_hyperfine2_start + microwave_hyperfine_pi_time / 2 + DDS_wait_time2
        ...
        DDS = [
            ...
            ('Microwave', WithUnit(microwave1_start_at, 'ms'), WithUnit(microwave_pi_time, 'ms'),
             ...
             WithUnit(0, 'dB')),
            # microwave_hyperfine
            ('Microwave', WithUnit(microwave_hyperfine1_start, 'ms'), WithUnit(microwave_hyperfine_pi_time / 2, 'ms'),
             WithUnit(scanfreq, 'MHz'), WithUnit(microwave_hyperfine_amp, 'dBm'), WithUnit(0.0, 'deg'),
             WithUnit(0, 'MHz'),
             WithUnit(0, 'dB')),
            ('Microwave', WithUnit(microwave_hyperfine2_start, 'ms'), WithUnit(microwave_hyperfine_pi_time / 2, 'ms'),
             WithUnit(scanfreq, 'MHz'), WithUnit(microwave_hyperfine_amp, 'dBm'), WithUnit(0.0, 'deg'),
             WithUnit(0, 'MHz'),
             WithUnit(0, 'dB')),
    ...
    def run(self, cxn, context, scanvalue, units):
        ...
        detuning = self.parameters.MicrowaveHyperfine.hpf_detuning
        Scan_Freq = self.parameters.MicrowaveHyperfine.hpf_resonance_freq - detuning
