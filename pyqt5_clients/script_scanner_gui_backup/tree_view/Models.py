...
from .Data import StringNode, SelectionSimpleNode, LineSelectionNode, SidebandElectorNode, EventNode, ListNode
...
class ParametersTreeModel(QtCore.QAbstractItemModel):
    ...
    def insert_list(self, parameter_name, info, parent_index):
        collectionNode = self.getNode(parent_index)
        row_count = self.rowCount(parent_index)
        self.beginInsertRows(parent_index, row_count, row_count)
        childNode = ListNode(parameter_name, info, collectionNode)
        self.endInsertRows()
        index = self.index(row_count, 0, parent_index)
        return index
