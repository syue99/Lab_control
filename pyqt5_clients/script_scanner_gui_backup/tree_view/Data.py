...
class ListNode(Node):
    ...
    def set_full_info(self, info):
        try:
            self._value = info#np.fromstring(info[1:-1], dtype=float, sep=",")
        except:
            pass
    ...
    def data(self, column):
        if column < 1:
            return super(ListNode, self).data(column)
        elif column == 1:
            return self.string_format()
        elif column == 2:
            return self._collection
    ...
    def full_parameter(self):
        ...
        if self._units:
            return ('scan', ([WithUnit(self._min, self._units), WithUnit(self._max, self._units)],
                             (WithUnit(self._scan_start, self._units), WithUnit(
                                 self._scan_stop, self._units), self._scan_points)
                             ))
        else:
...
class LineSelectionNode(Node):
    ...
    def full_parameter(self):
        return ('line_selection', (str(self._value), list(iteritems(self._dict))))
...
class SidebandElectorNode(Node):
    ...
    def full_parameter(self):
        return ('sideband_selection', [self._radial1,
                                       self._radial2, self._axial, self._micromotion])
...
class DurationBandwidthNode(Node):
    ...
    def full_parameter(self):
        if self._units:
            WithUnit = self.WithUnit
            return ('duration_bandwidth', [WithUnit(self._min, self._units), WithUnit(
                self._max, self._units), WithUnit(self._value, self._units)])
        else:
...
class SpectrumSensitivityNode(Node):
    ...
    def full_parameter(self):
        return ('spectrum_sensitivity',
                (self.WithUnit(self._span, 'kHz'), self.WithUnit(self._resolution, 'kHz'),
