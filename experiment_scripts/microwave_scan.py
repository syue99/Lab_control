...
class scan(experiment):
    ...
    def program_main_sequence(self,scantime = 2, scanfreq = 100):
        ...
        DDS = [('1092 Double Pass', WithUnit((doppler+1), 'ms'), WithUnit(scantime, 'ms'), WithUnit(scanfreq, 'MHz'), WithUnit(0, 'dBm'), WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),WithUnit(0, 'dB'))]
        # program DDS
        self.pulser.add_dds_pulses(DDS)
        self.pulser.add_ttl_pulse('TTL4', WithUnit(doppler+1+scantime, 'ms'), WithUnit(2, 'ms'))
        #trigger PMT
        for i in range(3):
            self.pulser.add_ttl_pulse('DiffCountTrigger',WithUnit(doppler+scantime+2*i, 'ms'),WithUnit(10.0, 'us'))
            self.pulser.add_ttl_pulse('DiffCountTrigger',WithUnit(doppler+scantime+2*(i+0.5), 'ms'),WithUnit(10.0, 'us'))
            self.pulser.add_ttl_pulse('866DP',WithUnit(doppler+scantime+2*i, 'ms'),WithUnit(1.0, 'ms'),)
            self.pulser.add_ttl_pulse('Internal866',WithUnit(doppler+scantime+2*i, 'ms'),WithUnit(1.0, 'ms'))
...
if __name__ == '__main__':
    ...
    exprt = scan_experiment_1D(scan, parameter, 90, 110, 500, 'MHz')
