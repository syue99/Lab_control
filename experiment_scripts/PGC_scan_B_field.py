...
#mot3d_B_off_time = 1
exposure_time = 1e-3
image_time = exposure_time+ 0.1
image_delay = 300e-3 ### delay after turning off MOT B field
image_num = 10
# mot_off1 = 0.2
# mot_off2 = 0.2
...
with labrad.connect() as cxn:
    ...
    duration = mot_loading_time + np.max([image_delay + image_num*image_time, 1]) +0.01 ### having at least 0.2s/1s turning off MOT
    ao = cxn.aoserver
    ...
    # ### change the detuning right after turning off MOT
    #ao.setvoltagepulse(offsetlock, mot_loading_time, image_num*image_time+ image_delay+6e-4, WithUnit(-5,"V"))
    ...
    for i in range(image_num):
        ttl.PulseOn(three_d_motAOM, mot_loading_time + i*image_time+ image_delay+6e-4, exposure_time) ### Turn on cooling/imaging and repump light 1ms before imaging
        ttl.PulseOn(repump_motAOM, mot_loading_time + i*image_time+ image_delay+6e-4, exposure_time)
