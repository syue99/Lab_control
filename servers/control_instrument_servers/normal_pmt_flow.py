...
class NormalPMTFlow(LabradServer):
    ...
    def setMode(self, c, mode):
        """Starts recording Time Resolved Counts into Data Vault."""
    ...
    def stopRecording(self, c):
        """Stops recording counts into Data Vault."""
        yield self.dostopRecording()
        otherListeners = self.getOtherListeners(c)
    ...
    def setTimeLength(self, c, timelength):
        ...
        initrunning = self.recording.running
    ...
    def _record(self):
        ...
        if len(rawdata) != 0:
            if self.currentMode == 'Normal':
                toDataVault = [[elem[2] - self.startTime, elem[0], 0, 0]
                               for elem in rawdata]
