...
class Pulser(DDS, LineTrigger):
    ...
    def addTTLPulse(self, c, channel, start, duration):
        """Adds a TTL Pulse to the sequence, times are in seconds."""
        if channel not in list(self.channelDict.keys()):
    ...
    def addTTLPulses(self, c, pulses):
        """Adds multiple TTL Pulses to the sequence, times are in seconds.
    ...
    def extendSequenceLength(self, c, timeLength):
        """Allows option to extend the total length of the sequence beyond the last TTL pulse."""
        sequence = c.get('sequence')
    ...
    def humanReadableTTL(self, c, getProgrammed=None):
        """Returns a readable form of TTL sequence.
    ...
    def switchManual(self, c, channelName, state=None):
        ...
        By default will go into the last remembered state but can also
        pass the argument which state it should go into.
        """
        if channelName not in list(self.channelDict.keys()):
    ...
    def repeatitionsCompleted(self, c):
        """Checks how many repeatitions have been completed in for the infinite or number modes."""
        yield self.inCommunication.acquire()
    ...
    def setMode(self, c, mode):
        ...
        """
        if mode not in list(self.collectionTime.keys()):
    ...
    def setCollectTime(self, c, new_time, mode):
        """Sets how long to collect photonslist.

        Sets in either 'Normal' or 'Differential' mode of operation."""
        new_time = new_time['s']
    ...
    def getALLCounts(self, c):
        ...
        Returns the list of counts stored on the FPGA in the form
        (v,s1,s2) where v is the count rate in KC/SEC
        and s can be 'ON' in normal mode or in Differential mode
        with 866 on and 'OFF' for differential
        mode when 866 is off. s2 is the approximate time of acquisition.
        NOTE: For some reason, FGPA ReadFromBlockPipeOut never time outs,
        so can not implement requesting more packets than
    ...
    def resetTimetags(self, c):
        """Resets the time resolved FIFO to clear any residual timetags."""
        yield self.inCommunication.acquire()
    ...
    def getTimetags(self, c):
        """Gets the time resolved timetags."""
        yield self.inCommunication.acquire()
