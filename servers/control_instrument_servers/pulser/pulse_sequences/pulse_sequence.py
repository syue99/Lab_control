...
class pulse_sequence(object):
    ...
    def __init__(self, parameter_dict, start=WithUnit(1.0, 'us')):
        ...
        The 1.0 microsecond is added for this reason and only affects
        the sequence with an initial 1 microsecond offset.
        """
        if not isinstance(parameter_dict, TreeDict):
            raise Exception("replacement_dict must be a TreeDict in sequence {0}".format(
    ...
    def all_required_parameters(cls):
        """Returns a list of all required variables for the current sequence.

        Also returns all used subsequences.
        """
    ...
    def fill_parameters(self, params, replace):
        if not len(params) == len(set(params)):
            raise Exception("Duplicate required parameters found in {0}".format(
                self.__class__.__name__))
        new_dict = TreeDict()
    ...
    def addDDS(self, channel, start, duration, frequency, amplitude, phase=WithUnit(
            0, 'deg'), ramp_rate=WithUnit(0, 'MHz'), amp_ramp_rate=WithUnit(0, 'dB')):
        """Adds a dds pulse to the pulse sequence."""
        ...
        self._dds_pulses.append((channel, start, duration, frequency,
                                 amplitude, phase, ramp_rate, amp_ramp_rate))

    def addTTL(self, channel, start, duration):
        """Adds a ttl pulse to the pulse sequence."""
    ...
    def addSequence(self, sequence, replacement_dict=TreeDict(), position=None):
        ...
        if sequence not in self.required_subsequences:
            raise Exception(
                "Adding subsequence {0} that is not listed in the required subequences".format(
                    sequence.__class__.__name__))
        if not isinstance(replacement_dict, TreeDict):
