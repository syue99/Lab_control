...
class LineTrigger(LabradServer):
    ...
    def initialize(self):
        ...
        self.linetrigger_limits = [WithUnit(v, 'us')
                                   for v in hardwareConfiguration.lineTriggerLimits]

    @setting(60, "Get Line Trigger Limits", returns='*v[us]')
    def getLineTriggerLimits(self, c):
        """Gets limits for duration of line triggering."""
    ...
    def line_trigger_state(self, c, enable=None):
        if enable is not None:
            ...
            self.notifyOtherListeners(
                c,
                (self.linetrigger_enabled,
                 self.linetrigger_duration),
    ...
    def line_trigger_duration(self, c, duration=None):
        """Enables or disables line triggering. If enabling, can specify the offset_duration."""
