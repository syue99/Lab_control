...
class DDS(LabradServer):
    ...
    def addDDSPulses(self, c, values):
        ...
        for value in values:
            ...
            if not channel.phase_coherent_model:
                ...
                # note that keeping the frequency the same when switching off to preserve
                # phase coherence
                num_off = self.settings_to_num(
                    channel, freq, ampl_off, phase, ramp_rate, amp_ramp_rate)
            # note < sign, because start can not be 0.
            # this would overwrite the 0 position of the ram, and cause the dds to
            # change before pulse sequence is launched
            if not self.sequenceTimeRange[0] < start <= self.sequenceTimeRange[1]:
                raise Exception(
                    ("DDS start time out of acceptable input range"
                     "for channel {0} at time {1}").format(name, start))
            if not self.sequenceTimeRange[0] < start + dur <= self.sequenceTimeRange[1]:
                raise Exception(
                    ("DDS start time out of acceptable input"
                     "range for channel {0} at time {1}").format(
                        name, start + dur))
    ...
    def output(self, c, name=None, state=None):
        """Turns the DDS on and off.

        Turning off the DDS sets the frequency and amplitude
    ...
    def _checkRange(self, t, channel, val):
        ...
        if not r[0] <= val <= r[1]:
            raise Exception(
                "channel {0} : {1} of {2} is outside the allowed range".format(
                    channel.name, t, val))
    ...
    def _setOutput(self, channel, state):
        if state and not channel.state:  # if turning on, and is currently off
            yield self.inCommunication.run(self._setParameters, channel,
                                           channel.frequency, channel.amplitude)
    ...
    def _getCurrentDDS(self):
        """Returns a dictionary {name:num} that gives the current DDS state.

        num is the num representation for DDS channel parameters.
        """
        d = dict([(name, self._channel_to_num(channel))
                  for (name, channel) in iteritems(self.ddsDict)])
        return d

    def _getCurrentDDSWithOffChannels(self, off_channels):
        """Returns a dictionary {name:num} that gives the current DDS state.

        num is the num representation for DDS channel parameters.
        The parameters for DDS off state are returned for DDS channels in the off_channels list.
        """
        d = dict([(name, self._channel_to_num(channel, name in off_channels))
                  for (name, channel) in iteritems(self.ddsDict)])
        return d

    def _channel_to_num(self, channel, return_off_parameter=False):
        """Returns the current state of the channel in the num representation.

        If return_off_parameter is True, returns the off state of the channel.
        """
        if channel.state and not return_off_parameter:
    ...
    def _valToInt_coherent(self, channel, freq, ampl, phase=0, ramp_rate=0, amp_ramp_rate=0):
        """Returns an integer representation of the dds setting.

        Takes the frequency and amplitude values for the specific channel.
        ...
        # if the ramp rate is smaller than the minim, then treat it as no ramp
        if ramp_rate < minim:
            seq = 0
        ...
        if (amp_ramp_rate < minim):
            ...
            seq_amp_ramp = int(np.ceil((slope - minim_slope) / resolution)
    ...
    def _intToBuf_coherent(self, num):
        """Takes the setting integer and returns the buffer string for dds programming."""

        # change according to the new DDS which supports 64 bit tuning of the
        # frequency. Used to be #freq_num = (num % 2**32)*2**32
        freq_num = (num % 2**64)
