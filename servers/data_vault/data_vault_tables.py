...
try:
	import tables
	print("PyTables imported")
except ImportError as e:
	print(e)
	print("PyTables not imported.")
## UM
...
class Session(object):
    ...
    def load(self):
        """Loads info from the session.ini file."""
        S = SafeConfigParser()
    ...
    def save(self):
        """Saves info to the session.ini file."""
        S = SafeConfigParser()
    ...
    def access(self):
        """Updates last access time and save."""
        self.accessed = datetime.now()
        self.save()

    def listContents(self, tagFilters):
        """Gets a list of directory names in this directory."""
        files = os.listdir(self.dir)
        dirs = [dsDecode(s[:-4]) for s in files if s.endswith('.dir')]
        datasets = [dsDecode(s[:-4]) for s in files if s.endswith('.csv') or s.endswith('.h5')] ## UM
        # apply tag filters
    ...
    def listDatasets(self):
        """Gets a list of dataset names in this directory."""
        files = os.listdir(self.dir)
        return [dsDecode(s[:-4]) for s in files if s.endswith('.csv') or s.endswith('.h5')] ## UM 
    ...
    def openDataset(self, name):
        ...
        if not (os.path.exists(os.path.join(self.dir, filename + '.csv'))) : ##UM
            raise DatasetNotFoundError(name)
...
class Image(object): ## UM TODO this should be all that we change
    def __init__(self, session, filename,filetype='.npy'):
        
        """session.dir is the dataset number to which this image should be attached."""
		## UM added the if-else block, filetype variable, extension variable
    ...
    def create_h5_directory(self,file,dir):
    
        dirList = dir.split('/')
        curNode = "/"
        for dirNodes in dirList:
            if (file.__contains__(curNode+dirNodes)):
                curNode=curNode+dirNodes+"/"
                continue
            group=file.create_group(curNode, dirNodes)
            curNode=curNode+dirNodes+"/"
    ...
    def add_data(self, data, imagedir):
		...
		## UM added the if-else structure and the code for pytables
        elif (self.extension == '.h5'):
            now = datetime.now()
            timestring = now.strftime("%Y%m%dT%H_%M_%S_%f")
            imagename = "image_"+timestring
            print("adding data")
            try:
                print(self.filename)
                fi = tables.open_file(self.filename, 'a')
            except:
            ## UM add actual error codes
                print("file does not exist, creating new file")	
                fi = tables.open_file(self.filename, 'w')
            finally:
                ...
                ## create node
                  #  group = fi.create_group("/", imagedir)
                     group = self.create_h5_directory(fi,imagedir)
                image = fi.create_carray(group, imagename, atom, data.shape) ## UM TODO add compression here
                print(imagename)
                image[:] = data
                fi.close()
	## UM function to open h5 files
    def get_image_array(self,imagename=None,imagedir='Images',rowrange=[None,None]):
    
        imagedir = '/'+imagedir+'/';
        try:
            fi = tables.open_file(self.filename,'a')
            if (imagename): ## open a user specified image
                nodedir = imagedir+imagename
                ...
                print(len(nodedir))
                print(self.imageID)
                if self.imageID>len(nodedirList): 
                    print("no more images")
                    return 0 ## no more images available
                
                
            imageNode = fi.get_node(nodedir) 
            print("node",imageNode)
            imagearray = imageNode.read(start=rowrange[0],stop=rowrange[1])
            print("array",imagearray)
            imagearray = numpy.float64(imagearray)
          ##  self.data = imagearray
        except: ## TODO add the exact errors
            print("Some error occured")		
        finally:
            fi.close()
        return imagearray
            
    ## returns a specified number of images
    def getData(self, limit, start):
        if limit is None:
        ##UM TODO see if there is a simpler fucntion in pyTables documentations
            self.imageID = 0
            self.data = self.get_image_array(imagename=None,imagedir='Images', rowrange=[None,None])
        else:
            self.data = numpy.zeros(shape=(limit-start,2,2))
            print(start,limit)
            for imageIdx in range(start,limit):
                print("imageIdx",imageIdx)
                print("imageID before call",self.imageID)
                self.data[imageIdx] = self.get_image_array(imagename=None,imagedir='Images', rowrange=[None,None])
                self.imageID+=1 ## update to get next image 
                print("imageID after call",self.imageID)
            print(numpy.shape(self.data))
        return self.data, start + len(self.data)
            
    ## UM just copied the streaming function from other classes
    def keepStreamingImages(self, context,pos):
        if pos < len(self.data):
            if context in self.listeners:
                self.listeners.remove(context)
            self.parent.onImageArrayRead(None, context)
        else:
...
class Dataset:
    def __init__(self, session, name, dtype=None, title=None, num=None,
                 create=False, ftype='.csv'): ## UM added ftype variable
        self.parent = session.parent
        self.name = name
        self.session = session  # MK
        file_base = os.path.join(session.dir, dsEncode(name))
        self.datafile = file_base + ftype ## UM 
    ...
    def access(self):
        """Updatse time of last access for this dataset."""
        self.accessed = datetime.now()
        self.save()
    ...
    def file(self):
        ...
        The file is also scheduled to be closed if it has not accessed for a while.
        """
        if not hasattr(self, '_file'):
            self._file = open(self.datafile, 'a+')  # append data ## UM TODO do we need to change this?
    ...
    def addIndependent(self, label):
        """Adds an independent variable to this dataset."""
    ...
    def addDependent(self, label):
        """Adds a dependent variable to this dataset."""
        if isinstance(label, tuple):
    ...
    def addParameter(self, name, data, saveNow=True):
        ...
        self.parent.onNewParameterDataset((int(self.name[0:5]), self.name[8:len(
            self.name)], self.session.path, name), self.parent.root.listeners)
        self.param_listeners = set()
        return name
...
class DataVault(LabradServer):
    ...
    def initServer(self):
        ...
        try:
            ...
            try:
                ...
                print('Please enter data storage directory or hit enter'
    ...
    def expireContext(self, c):
        """Stops sending any signals to this context."""
    ...
    def getSession(self, c):
        """Gets a session object for the current path."""
        return Session(c['path'], self) ## Session(dir, parent). Keeps a log of the datasets in given directory

    def getDataset(self, c):
        """Gets a dataset object for the current dataset."""
        if 'dataset' not in c: # checks if dataset is available in context
            raise NoDatasetError()
        session = self.getSession(c)
        return session.datasets[c['dataset']] # returns the dataset in current context
    ...
    onNewAdditionalHeaderDataset = Signal(
        543627, 'signal: new additional header dataset', '(i, s, ?, s)')
    onCommentsAvailable = Signal(543621, 'signal: comments available', '')
    
    #image signals
    onImageArrayRead = Signal(543630, 'signal: image array available', '')
    ...
    def dir(self, c, tagFilters=['-trash'], includeTags=None):
        """Gets subdirectories and datasets in the current directory."""
    ...
    def mkdir(self, c, name):
        """Makes a new sub-directory in the current directory.
    ...
    def open_appendable(self, c, name):
        """Opens a Dataset for reading and appending.
    ...
    def add(self, c, data):
        """Adds data to the current dataset.
    ...
    @setting(22, data='*2v', image_size='*i', repetitions='i', filename='s',filetype='s',
	         imagedir='s', returns='') ## UM changed data to *2i from *i
    def save_image(self, c, data, image_size, repetitions, filename,imagedir='/', filetype='.npy'): 
        """Saves a CCD image of the open dataest to a .npy file."""
        session = self.getSession(c)
        x_pixels, y_pixels = image_size
      #  data = numpy.reshape(data, (repetitions, y_pixels, x_pixels)) #UM commented this line because the reshaping does not make sense
        image = Image(session, filename, filetype)
        image.add_data(data, imagedir)
    ...
    def stream_h5image(self, c,filename, limit=None, startOver=True, rowrange=[None,None]):
        ...
        Limit is the maximum number of images to return, with
        the default being to return the whole dataset.  Setting the
        startOver flag to true will return data starting at the beginning
        of the dataset.  By default, only new data that has not been seen
        in this context is returned.
        ...
        c['filepos'] = 0 if startOver else c['filepos'] ## refers to the image index
        data, c['filepos'] = image.getData(limit, c['filepos'])
        image.keepStreamingImages(c.ID,c['filepos'])

        return data

    @setting(100, returns='(*(ss){independents}, *(sss){dependents})')
    def variables(self, c):
        """Gets the independent and dependent variables for the current dataset.
    ...
    @setting(101, returns='*(ss)')
    def config_info(self, c):
        """Gets a list of tuples (label, value) general section of dataset."""
        ds = self.getDataset(c)
        info = []
        info.append(('dtype', ds.dtype))
        info.append(('created', timeToStr(ds.created)))
        info.append(('accessed', timeToStr(ds.accessed)))
        info.append(('modified', timeToStr(ds.modified)))
        info.append(('title', ds.title))
        return info
    ...
    def add_parameter(self, c, name, data):
        """Adds a new parameter to the current dataset."""
        dataset = self.getDataset(c)
    ...
    def add_comment(self, c, comment, user='anonymous'):
        """Adds a comment to the current dataset."""
        dataset = self.getDataset(c)
    ...
    def get_comments(self, c, limit=None, startOver=False):
        """Gets comments for the current dataset."""
        dataset = self.getDataset(c)
    ...
    def update_tags(self, c, tags, dirs, datasets=None):
        """Updates the tags for the specified directories and datasets.
    ...
    def get_tags(self, c, dirs, datasets):
        """Gets tags for directories and datasets in the current dir."""
        sess = self.getSession(c)
        if isinstance(dirs, str):
            dirs = [dirs]
    ...
    def add_additional_header(self, c, header_name, name, data):
        """Adds a new additional header to the current dataset.
    ...
    def additional_headers(self, c):
        """Gets a list of additional header (header_name, name) tuples."""
        dataset = self.getDataset(c)
    ...
    def get_additional_header(self, c, header_name, name, case_sensitive=True):
        """Gets the value of an additional header."""
        dataset = self.getDataset(c)
