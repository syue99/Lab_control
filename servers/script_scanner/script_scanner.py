...
class ScriptScanner(ScriptSignalsServer):
    ...
    def get_progress(self, c, script_ID):
        """Gets progress of a currently running experiment."""
    ...
    def new_experiment(self, c, script_name):
        """Queues an experiment for launching.

        Returns the scan ID of the queued experiment from a scheduler instance.
    ...
    def cancel_scheduled_script(self, c, scheduled_ID):
        """Cancels the currently scheduled script."""
    ...
    def pause_or_stop(self, c, script_ID):
        """Returns the boolean whether or not the script should be stopped.

        This request blocks while the script is to be paused.
        """
        status = self.scheduler.get_running_status(script_ID)
        if status is None:
    ...
    @inlineCallbacks
    def stopServer(self):
        """Stops all the running scripts and exit."""
        yield None
        try:
