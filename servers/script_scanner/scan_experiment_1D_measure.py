...
##REVISED BY FRED TO MAKE SAVING PARAMETERS MORE APPROPRIATE
...
class scan_experiment_1D_measure(experiment):
    ...
    #Revised by Fred
        return 'Scanning {0} in {1} while {2}'.format(
            self.scan_script_cls.parameter_name, scan_name, measure_name)
...
#Revised by Fred by adding parameter
    def navigate_data_vault(self, cxn, parameter, context):
        ...
        dv.new(dataset_name, [(self.scan_script.parameter_name, self.units)], [
               (self.measure_script.parameter_name, self.measure_script.parameter_explanation, self.measure_script.parameter_unit)], context=context)
