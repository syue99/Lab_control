...
class scheduler(object):
    ...
    def running_deferred_list(self):
        return [script.defer_on_done for script in itervalues(
            self.running) if not script.externally_launched]
    ...
    def remove_queued_script(self, script_ID):
        ...
        if not removed:
            raise Exception(
                "Trying to remove scirpt ID {0} from queue but it's not in the queue".format(
                    script_ID))

    def add_scan_to_queue(self, scan, priority='Normal'):
        """Called by scriptscanner.new_experiment.

        self.scan_ID_counter advances by 1 each time this is called, giving each
        new experiment that will be run a distinct number.
    ...
    def is_higher_priority_than_running(self, priority):
        try:
            priorities = sorted([running.priority for running in itervalues(self.running)])
    ...
    def change_period_scheduled_script(self, scheduled_ID, period):
        try:
            ...
            raise Exception(
                "Schedule Script {0} with {1} ID does not exist".format(name, scheduled_ID))
        else:
            lc.stop()
