...
class scan_experiment_1D_sc(experiment):
    ...
    def __init__(self, script_cls, parameter, minim, maxim, steps, units):
        ...
        super(scan_experiment_1D_sc, self).__init__(scan_name)
    ...
    def run(self, cxn, context, **kwargs):
        for i, scan_value in enumerate(self.scan_points):
            ...
            result = self.script.run(cxn, context, scan_value, self.units)
...
#Revised by Fred by adding parameter
    ...
    def finalize(self, cxn, context):
        ...
        if self.raw_data !=[]:
            ...
            np.savetxt("C:/Users/Administrator/Desktop/Lab_control-Development/servers/data_vault/__data__/" + self.dirc + "raw_data.csv", self.raw_data, delimiter=",",
                       fmt="%s")
