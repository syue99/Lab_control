...
def arbitrarySegmentsByIdxSameCol(args):
    ...
    f_delta_col: AOD freq delta relative to start index frequency.
        Single number or list of length n_segments + 1. If single value, use same freq for all segments.
    f_delta_row: AOD freq delta relative to start index frequency. List of n_tweezers_moved,
        or list of [list of n_tweezers_moved].
    ...
    At least one of {f_delta_row, f_delta_col, power, dur_row, dur_col} must be a list of length n_segments + 1.
    """
    ...
    vectorizedArgs = {}
    for i in segmentParamsList:
        if i == 'f_delta_row':  # broadcast into 2D array
            assert (isinstance(args[i], list) or isinstance(args[i], np.array))
            if isinstance(args[i][0], numbers.Number):
                segmentParamsList[i] = np.kron(np.ones((length, 1)), np.array(args[i]))
            else:
                assert (len(args[i]) == length)
                segmentParamsList[i] = np.array(args[i])
            # Add dummy_row
            segmentParamsList[i] = np.concatenate((segmentParamsList[i], np.kron(np.ones((length, 1)), dummpy_row)),
                                                  axis=1)
        else:
            vectorizedArgs[i] = singleValue2list(args[i], length)

    vectorizedArgs['f_delta_row'] += row_start
    vectorizedArgs['f_delta_col'] += col_start

    print(vectorizedArgs)
    move_list = []
    for i in range(length - 1):
        config = {'f0_row': vectorizedArgs['f_delta_row'][i] + row_start,
                  'f1_row': vectorizedArgs['f_delta_row'][i + 1] + row_start,
                  'f0_col': vectorizedArgs['f_delta_col'][i] + col_start,
                  'f1_col': vectorizedArgs['f_delta_col'][i + 1] + col_start,
                  'a0_row': a_row,
                  'a1_row': a_row,
                  'a0_col': a_col,
                  'a1_col': a_col,
                  'p0': vectorizedArgs['power'][i],
                  'p1': vectorizedArgs['power'][i + 1],
                  'phase_col': phase_col,
                  'phase_row': phase_row,
                  'dur_row': vectorizedArgs['dur_row'][i],
                  'dur_col': vectorizedArgs['dur_col'][i]}
        move_i = Move(**config)
        move_list.append(move_i)
