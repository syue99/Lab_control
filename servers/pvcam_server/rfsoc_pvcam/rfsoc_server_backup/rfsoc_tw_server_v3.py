...
class RFSoCProgramServer(LabradServer):
    ...
    def run_exp(self, c, pdata):
        ...
        if self.max_moves != data['max_moves']:
            raise RuntimeError("The max_moves sent via run_exp (=%d) is not equal to the max_moves "
                               "specified when socket server was started (=%d)!" % (data['max_moves'], self.max_moves))
        config = {
    ...
    def run_exp_internal(self, c, pdata='y'):
        ...
        if self.max_moves != data['max_moves']:
            ...
            raise RuntimeError("The max_moves sent via run_exp_internal (=%d) is not equal to the max_moves "
                               "specified when socket server was started (=%d)!" % (data['max_moves'], self.max_moves))
        config = {
            'max_moves': self.max_moves,
            'states' : self.states,
...
#             print("select.select time:", (time.time()-t0))
    ...
    def init_buffers(self):
        ...
        # profiled, DMA was considerably faster than mmio via single_write.
        # This will be used for m_state, m_next_state and one element of
        # n_moves_lookup. Even though we only need length 1, we alloc length 2
        # since sometimes buffer allocations are more robust if length is
        # integer multiple of 64 bits. 
        self.aux_buff = allocate(shape=(2,), dtype=np.int32)
