...
from pvcam_Config import pvcam_Config as config

class pvcamVideo(QtGui.QWidget):
    def __init__(self, server):
        super(pvcamVideo, self).__init__()
...
#        self.emccdSpinBox.setMaximum(emrange[1])
    ...
    def setup_layout(self):
        ...
        # # emrange= yield self.server.getemrange(None)
        # self.emrange = [0,0]# FRED: yield self.server.getemrange(None)
    ...
    def save_image(self, image_data):
        if not _np.array_equal(image_data, self.saved_data):
            ...
            if self.save_format == "tsv":
                _np.savetxt(path + ".tsv", saved_data_in_int, fmt='%i', header=header)
            elif self.save_format == "csv":
                _np.savetxt(path + ".csv", saved_data_in_int, fmt='%i', delimiter=",",
                            header=header)
    ...
    def datetime_to_str_list(self):
        dt = datetime.now()
        dt_str = [str(dt.year).rjust(4, "0"), str(dt.month).rjust(2, "0"),
                  str(dt.day).rjust(2, "0"), str(dt.hour).rjust(2, "0"),
...
class image_region_selection_dialog(QtGui.QDialog):
    ...
    def set_image_region(self, bin_hor, bin_ver, start_hor, stop_hor, start_ver, stop_ver):
        yield self.server.set_image_region(bin_hor, bin_ver, start_hor,
                                           stop_hor, start_ver, stop_ver)

    @inlineCallbacks
    def setupLayout(self):
        self.hor_max, self.ver_max = yield self.server.get_detector_dimensions(None)
        self.hor_min, self.ver_min = [1, 1]
        cur_bin_hor, cur_bin_ver, cur_start_hor, cur_stop_hor, cur_start_ver, \
            cur_stop_ver = yield self.server.getImageRegion(None)
        layout = QtGui.QGridLayout()
    ...
    def do_submit(self, bin_hor, bin_ver, start_hor, stop_hor, start_ver, stop_ver):
        if self.parent.live_update_loop.running:
            ...
            try:
                yield self.server.setImageRegion(None, bin_hor, bin_ver, start_hor,
                                                 stop_hor, start_ver, stop_ver)
            except Exception as e:
