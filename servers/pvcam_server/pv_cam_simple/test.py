import ctypes as c
#from pydux.lib.config.andor_config import andor_config as config
...
class AndorCamera(object):
    """Provides the Python version of the same functions that are defined in the Andor's SDK.

    Since Python does not have pass by reference for immutable variables, some
    of these variables are actually stored in the class instance. For example
    the temperature, gain, gainRange, status etc. are stored in the class.
    """

    def __init__(self):
        #print(config.path_to_dll)
        a = 0
        if (a==0):
            print('Loading DLL')
            if os.name == "nt":
            ...
            num = c.c_ulong()
            self.dll.GetAvailableCameras(c.byref(num))
            print(num)
            ...
            self.set_image(config.binning[0], config.binning[0],
                           1, self.info.width, 1, self.info.height)
            self.set_cooler_on()
    ...
    def print_get_software_version(self):
        ...
        dllRev = c.c_int()
        dllVer = c.c_int()
        self.dll.GetSoftwareVersion(c.byref(eprom), c.byref(cofFile), c.byref(
            vxdRev), c.byref(vxdVer), c.byref(dllRev), c.byref(dllVer))
    ...
    def get_temperature(self):
        ...
        if ((ERROR_CODE[error] == 'DRV_TEMP_STABILIZED'
             or ERROR_CODE[error] == 'DRV_TEMP_NOT_REACHED'
             or ERROR_CODE[error] == 'DRV_TEMP_DRIFT'
             or ERROR_CODE[error] == 'DRV_TEMP_NOT_STABILIZED')):
    ...
    def get_acquisition_timings(self):
        ...
        error = self.dll.GetAcquisitionTimings(
            c.byref(exposure), c.byref(accumulate), c.byref(kinetic))
        if (ERROR_CODE[error] == 'DRV_SUCCESS'):
    ...
    def set_image(self, hbin, vbin, hstart, hend, vstart, vend):
        ...
        vstart = int(vstart)
        vend = int(vend)
        error = self.dll.SetImage(c.c_int(hbin), c.c_int(vbin), c.c_int(
            hstart), c.c_int(hend), c.c_int(vstart), c.c_int(vend))
        if (ERROR_CODE[error] == 'DRV_SUCCESS'):
