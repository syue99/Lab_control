...
from pvcam_video import pvcamVideo
...
class pvcamServer(LabradServer):
    ...
    def initServer(self):
        self.listeners = set()
        self.camera = PvcamDriver().cam
        self.lock = DeferredLock()
        #self.gui = pvcamVideo(self)
...
#simple methods to consier


    @setting(23, "Get Most Recent Image", returns='*v')
...
# #         print 'acquiring: {}'.format(self.getMostRecentImage.__name__)
        ...
        try:
            ...
            image = yield deferToThread(self.camera.get_frame)
        finally:             #             print 'releasing: {}'.format(self.getMostRecentImage.__name__)
            self.lock.release()
        returnValue(image)
...
#             self.lock.release()
#         # need to request the actual set value because it may differ from the
#         # request when the request is not possible
#         time = self.camera.get_exposure_time()
#         if c is not None:
...
#     @setting(17, "Set Image Region", horizontalBinning='i', verticalBinning='i',
#              horizontalStart='i', horizontalEnd='i', verticalStart='i', verticalEnd='i', returns='')
...
#                                 verticalBinning, horizontalStart, horizontalEnd,
...
#         """Gets the counts with the vertical axis summed over."""
...
#             images = _np.reshape(images, (num_images, y_pixels, x_pixels))
#             images = images.sum(axis=1)
#             images = _np.ravel(images, order='C')
...
#     @setting(34, regions='*(iiii)', num_images='i', returns='*v[]')
#     def average_region(self, c, regions, num_images=1):
#         """Averaged counts in regions specified by list of (xstart, xend, ystart, yend)."""
#         images = []
#         while len(images) < num_images:
#             image = yield self.getMostRecentImage(c)
#             image = _np.array(image)
#             if len(images) == 0 or not (image == images[-1]).all():
#                 images.append(image)
#         hbin, vbin, hstart, hend, vstart, vend = self.camera.get_image()
#         x_pixels = int((hend - hstart + 1.) / (hbin))
#         y_pixels = int((vend - vstart + 1.) / (vbin))
...
#         avg_counts = []
#         for kk in regions:
#             slices = images[:, kk[2]:kk[3], kk[0]:kk[1]]
#             avg_counts.append(_np.mean(slices))
#         returnValue(avg_counts)
...
#                 # useful for debugging of how many iterations have been completed in case
#                 # of missed trigger pulses
...
#         """Starts the loop sending images to remote clients."""
#         self.live_update_loop = LoopingCall(
#             self.update_signal_loop)  # loop to send images to remote clients
#         self.last_image = None  # the last retrieved image
#         # a reasonable interval considering the Network speed, setting it shorter
#         # should not negatively influence the performance
#         self.live_update_loop.start(0.1)

#     @setting(202, returns='')
#     def stop_signal_loop(self, c):
#         """Stops the loop sending images to remote clients."""
