...
class single_pi_pulse(phase):
    phase_name = "single_pi_pulse"
    parameter_names = ["uv_duration_sp"]
    external_parameter_names = [("general","efieldvz"),("general","efieldvy"),("general","efieldvx"),("general","scan_variable"),("general","scan_variable_name"),
                                ...
                                ("uv_general","red_high_duration"),("uv_general","tweezer_extra_off_time"),("uv_general","uv_on_delay")]

    def _initialize(self, cxn, params):
        pass
        # if params["uv_duration_sp"]["s"]<=0:
        ...
        # cxn.keysight_33600a.single_pulse(0.78,params["uv_duration_sp"]["s"]*1e9,0.0,1)  ### SET AT 0.78V
    ...
    ####@@@@!!! DDS needs to be fully defined over the sequence
        ...
        uv_release_time = params["tweezer_extra_off_time"]["s"]+params["uv_duration_sp"]["s"]
        phase_time = np.ceil((uv_release_time)*1e6 / 4) * 4/1e6

        if self.tstart < dds_start_time_delay['s']:
            #as this phase need to borrow time from previous phase, so if it is the first phase one need to add some time here
            self.tstart = params["uv_on_delay"]["s"] + params["red_high_duration"]["s"]

            DDS.append((offsetlock, WithUnit(0.5,'us'), ### @@@ dds start time delay
            WithUnit(phase_time+self.tstart,"s")- dds_start_time_delay - WithUnit(0.5,'us'),  #### @@@?? WithUnit(tweezer_ramp_down_time*2+coolingimg_time, 's'), why *2??
            params["coolingdetune_for_fast_pushout"], 
            offsetlock_amp,
            WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),WithUnit(0, 'dB')))
            cxn.finitedopulses.PulseOn(dds_trigger, 0, 1e-5)
            cxn.pulser.add_ttl_pulse('TTL2', WithUnit(0.1, 'us'), WithUnit(0.1, 'us')) #### @@@??
        ...
        cxn.finitedopulses.PulseOn(red_aom_switch_ttl, self.tstart - params["uv_on_delay"]["s"] - params["red_high_duration"]["s"]*0.8, params["red_high_duration"]["s"])
        cxn.finitedopulses.PulseOn(uv_aom, self.tstart - params["uv_on_delay"]["s"] , np.max([params["uv_duration_sp"]["s"], 3e-7]))

        cxn.AOServer.setvoltagepulse(tweezer_slm_amp, self.tstart, phase_time, WithUnit(0,"V"))
        
        #magnetic field        
        cxn.finitedopulses.PulseOn(bias_y_switch_ttl, self.tstart, phase_time)
        cxn.AOServer.setvoltagepulse(bias_fieldy, self.tstart, phase_time, params["bias_yv_for_nulling"])
        cxn.AOServer.setvoltagepulse(bias_fieldx, self.tstart, phase_time, params["bias_xv_for_op_quant_axis"])

        cxn.finitedopulses.PulseOn(biasBz_forward_ttl, self.tstart, phase_time)
        cxn.AOServer.setvoltagepulse(bias_fieldz, self.tstart, phase_time, params["bias_zv_for_op_quant_axis"])
    ...
    def getlength(self, params):
        return np.ceil(((params["tweezer_extra_off_time"]["s"]+params["uv_duration_sp"]["s"])  *1e6 / 4)) * 4/1e6
...
def exp_sequence():
    return S(single_pi_pulse())#,imaging())


class single_pi_pulse_exp(experiment_phases):
    #for all parameters that are used in the experiment but not in the phases included
    required_parameters = single_pi_pulse.external_parameter_names + [("general","verbose"),("general","seqlen"),("general","nLoops"),("uv_general","tweezer_extra_off_time")]
    name = single_pi_pulse.phase_name
    first = None
    phases = None
    last = None
...
if __name__ == '__main__':
    ...
    exprt = single_sequence(single_pi_pulse_exp)
