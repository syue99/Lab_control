...
class thermalize(phase):
    phase_name = "thermalize"
    parameter_names = ["thermalize_time","tweezer_waiting_amp"]
    external_parameter_names = [("general","efieldvz"),("general","efieldvy"),("general","efieldvx"),("general","scan_variable"),("general","scan_variable_name"),
                                ("load_MOT_Tweezers","coolingamp_for_loading"),("load_MOT_Tweezers","bias_xv_for_loading"),("load_MOT_Tweezers","bias_yv_for_loading"),("load_MOT_Tweezers","bias_zv_for_loading")]
    
    def dophase(self, cxn, params):
    ################
    ##### Phase3B: image with PGC config
    ####@@@@!!! DDS needs to be fully defined over the sequence
        thermalize_time = params["thermalize_time"]['s']

        cxn.finitedopulses.PulseOn(tweezer_slm_AOM, self.tstart, thermalize_time)
        cxn.finitedopulses.PulseOn(tweezer_slm_aom_pid, self.tstart, thermalize_time)
        cxn.AOServer.setvoltagepulse(tweezer_slm_amp, self.tstart, thermalize_time, params["tweezer_waiting_amp"])
        
        cxn.AOServer.setvoltagepulse(three_d_motAOM_amp, self.tstart, thermalize_time, params["coolingamp_for_loading"])

        ######second cooling with lowered trap with extra_cooling_time
        cxn.finitedopulses.PulseOn(three_d_motAOM, self.tstart, thermalize_time) ####@changed from pgc_cooling_time*2 to pgc_cooling_time
        cxn.finitedopulses.PulseOn(repump_motAOM, self.tstart, thermalize_time)
        #magnetic field        
        cxn.AOServer.setvoltagepulse(bias_fieldx, self.tstart, thermalize_time, params["bias_xv_for_loading"]) ##
        cxn.AOServer.setvoltagepulse(bias_fieldy, self.tstart, thermalize_time,  params["bias_yv_for_loading"])
        cxn.finitedopulses.PulseOn(biasBz_forward_ttl, self.tstart, thermalize_time)
        cxn.AOServer.setvoltagepulse(bias_fieldz, self.tstart, thermalize_time, params["bias_zv_for_loading"]) ## 

        


    def getlength(self, params):
        return params["thermalize_time"]['s']
...
def exp_sequence():
    return S(thermalize())#,imaging())


class thermalize_exp(experiment_phases):
    #for all parameters that are used in the experiment but not in the phases included
    required_parameters = thermalize.external_parameter_names + [("general","verbose"),("general","seqlen"),("general","nLoops")]
    name = thermalize.phase_name
    first = None
    phases = None
    last = None
...
if __name__ == '__main__':
    ...
    exprt = single_sequence(thermalize_exp)
