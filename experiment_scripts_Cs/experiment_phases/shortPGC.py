...
class shortPGC(phase):
    phase_name = "shortPGC"
    parameter_names = ["img_field_pgc_cooling_time","img_field_coolingdetune_for_pgc","img_field_coolingamp_for_pgc"]
    external_parameter_names = [("general","efieldvz"),("general","efieldvy"),("general","efieldvx"),("general","scan_variable"),("general","scan_variable_name"),
                                ("imaging","bias_yv_for_nulling"),("imaging","bias_xv_for_nulling"),("imaging","bias_zv_for_nulling"),("imaging","tweezer_img_amp")]
    
    def dophase(self, cxn, params):
    ################
    ##### Phase4: short PGC at img field
    ####@@@@!!! DDS needs to be fully defined over the sequence
        ...
        img_field_pgc_cooling_time = params["img_field_pgc_cooling_time"]['s']
        #print(self.tstart)'
        if self.tstart < dds_start_time_delay['s']:
            DDS.append((offsetlock, WithUnit(self.tstart+0.5,'us'), ### @@@ dds start time delay
                        params["img_field_pgc_cooling_time"]- dds_start_time_delay - WithUnit(0.5,'us'), 
                        params["img_field_coolingdetune_for_pgc"], 
                        offsetlock_amp,
            ...
            DDS.append((offsetlock, WithUnit(self.tstart,'s') - dds_start_time_delay, ### @@@ dds start time delay
                        params["img_field_pgc_cooling_time"],  #### @@@?? WithUnit(tweezer_ramp_down_time*2+coolingimg_time, 's'), why *2??
                        params["img_field_coolingdetune_for_pgc"], 
                        offsetlock_amp,
                        WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),WithUnit(0, 'dB')))
        cxn.finitedopulses.PulseOn(tweezer_slm_AOM, self.tstart, img_field_pgc_cooling_time)
        cxn.finitedopulses.PulseOn(tweezer_slm_aom_pid, self.tstart, img_field_pgc_cooling_time)
        cxn.AOServer.setvoltagepulse(tweezer_slm_amp, self.tstart, img_field_pgc_cooling_time, params["tweezer_img_amp"])
        
        cxn.AOServer.setvoltagepulse(three_d_motAOM_amp, self.tstart, img_field_pgc_cooling_time, params["img_field_coolingamp_for_pgc"])

        ######second cooling with lowered trap with extra_cooling_time
        cxn.finitedopulses.PulseOn(three_d_motAOM, self.tstart, img_field_pgc_cooling_time) ####@changed from pgc_cooling_time*2 to pgc_cooling_time
        cxn.finitedopulses.PulseOn(repump_motAOM, self.tstart, img_field_pgc_cooling_time)
        #magnetic field        
        cxn.AOServer.setvoltagepulse(bias_fieldx, self.tstart, img_field_pgc_cooling_time, params["bias_xv_for_nulling"]) ##
        cxn.AOServer.setvoltagepulse(bias_fieldy, self.tstart, img_field_pgc_cooling_time,  params["bias_yv_for_nulling"])
        cxn.finitedopulses.PulseOn(biasBz_forward_ttl, self.tstart, img_field_pgc_cooling_time)
        cxn.AOServer.setvoltagepulse(bias_fieldz, self.tstart, img_field_pgc_cooling_time, params["bias_zv_for_nulling"]) ## 
    ...
    def getlength(self, params):
        return params["img_field_pgc_cooling_time"]['s']
...
def exp_sequence():
    return S(shortPGC())#,imaging())


class shortPGC_exp(experiment_phases):
    #for all parameters that are used in the experiment but not in the phases included
    required_parameters = shortPGC.external_parameter_names + [("general","verbose"),("general","seqlen"),("general","nLoops")]
    name = shortPGC.phase_name
    first = None
    phases = None
    last = None
...
if __name__ == '__main__':
    ...
    exprt = single_sequence(shortPGC_exp)
