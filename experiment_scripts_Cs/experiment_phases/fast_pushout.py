...
class fast_pushout(phase):
    phase_name = "fast_pushout"
    parameter_names = ["coolingdetune_for_fast_pushout","fastpush_out_time","push_out_amp_fastpush"]
    ...
    ####@@@@!!! DDS needs to be fully defined over the sequence
        ...
        phase_time = np.ceil(((params["fastpush_out_time"]['s']+2e-6)*1e6 / 4)) * 4/1e6
        if self.tstart < dds_start_time_delay['s']:
            ...
            DDS.append((OP_dds, WithUnit(self.tstart+0.5-2,'us') + WithUnit(phase_time,"s") - params["fastpush_out_time"],
                        params["fastpush_out_time"]+ WithUnit(2e-6, 's')- dds_start_time_delay - WithUnit(0.5,'us'), 
                        opAOM_freq, params["push_out_amp_fastpush"], WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),WithUnit(0, 'dB')))  ### @@@?? offsetlock_amp does it vary
            ...
            DDS.append((OP_dds, WithUnit(self.tstart-2e-6,'s') -dds_start_time_delay + WithUnit(phase_time,"s") - params["fastpush_out_time"],
                        params["fastpush_out_time"]+ WithUnit(2e-6, 's'), 
                        opAOM_freq, params["push_out_amp_fastpush"], WithUnit(0.0, 'deg'), WithUnit(0, 'MHz'),WithUnit(0, 'dB')))  ### @@@?? offsetlock_amp does it vary

        cxn.finitedopulses.PulseOn(OP_dds_switch, self.tstart+ phase_time - params["fastpush_out_time"]['s']-1e-6, params["fastpush_out_time"]['s'])                      
        cxn.finitedopulses.PulseOn(repump_motAOM, self.tstart+ phase_time - params["fastpush_out_time"]['s']-1e-6, params["fastpush_out_time"]['s'])
    ...
    def getlength(self, params):
        return np.ceil(((params["fastpush_out_time"]['s']+2e-6)*1e6 / 4)) * 4/1e6
...
def exp_sequence():
    return S(fast_pushout())#,imaging())


class fast_pushout_exp(experiment_phases):
    #for all parameters that are used in the experiment but not in the phases included
    required_parameters = fast_pushout.external_parameter_names + [("general","verbose"),("general","seqlen"),("general","nLoops")]
    name = fast_pushout.phase_name
    first = None
    phases = None
    last = None
...
if __name__ == '__main__':
    ...
    exprt = single_sequence(fast_pushout_exp)
